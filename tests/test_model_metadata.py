...
def test_load_model_metadata_reads_architecture_fields(monkeypatch):
    monkeypatch.setattr(
    ...
    meta = load_model_metadata("nomic-embed-text")

    assert meta.context_length == 2048
    assert meta.embedding_dim == 768
    assert meta.capabilites == []


def test_load_model_metadata_raises_when_show_fails(monkeypatch):
    def fail(name):
    ...
    with pytest.raises(ModelInfoError, match="nomic-embed-text"):
        load_model_metadata("nomic-embed-text")


def test_load_model_metadata_raises_when_fields_are_missing(monkeypatch):
    monkeypatch.setattr(
        "v1.app.ingestion.vector.utils.utils.ollama.show",
    ...
    with pytest.raises(ModelInfoError, match="did not expose"):
        load_model_metadata("llama3.2:3b")
